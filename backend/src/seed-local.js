...
const CSV_PATH = process.env.LOCAL_CSV_PATH ||
  path.join(__dirname, '..', 'data', 'karachi_local_bus_stops.csv');

const SERVICE_TYPE = 'local';
const ROUTE_PREFIX = 'local-';
...
const connectDB = async () => {
  ...
  console.log('MongoDB connected for local bus seeding');
};
...
const seed = async () => {
  ...
  try {
    ...
    console.log(`Clearing existing ${SERVICE_TYPE} data...`);
    await Stop.deleteMany({ serviceType: SERVICE_TYPE });
    await Route.deleteMany({ serviceType: SERVICE_TYPE });
    ...
    rawRows.forEach((row, i) => {
      ...
      validRows.push({
        ...
        routeType: (row.route_type || '').trim() || 'Local Bus',
    ...
    console.log(`Valid rows: ${validRows.length}, Skipped: ${stats.skippedRows}`);
    ...
    for (const [, rows] of byName) {
      ...
      if (clusters.length > 1) {
        ...
        stats.warnings.push(
          ...
          `(${distinctCoordPairs} unique coordinate pairs, spread beyond ${MERGE_TOLERANCE_METERS}m) - split.`
        );
      }
      ...
      clusters.forEach((cluster) => {
        ...
        const finalName = clusters.length > 1
          ...
          : baseName;
        const resolvedKey = `${finalName}::${SERVICE_TYPE}`;
    ...
    for (const row of validRows) {
      ...
      if (!resolvedKey || !createdStopIds.has(resolvedKey)) continue;
    ...
    for (const [code, routeData] of routeMap) {
      ...
      if (duplicateOrders.length > 0) {
        stats.warnings.push(`Route ${code}: duplicate stop_order(s) [${[...new Set(duplicateOrders)].join(', ')}] - preserved as listed.`);
      }
      ...
      if (orderedStopIds.length === 0) {
        stats.routesSkipped++;
        stats.warnings.push(`Route ${code}: no valid stops - skipped`);
        continue;
      }
